...
namespace Stand_Launchpad
{
	...
	public partial class Launchpad : Form
	{
        ...
        private int checkForUpdate(bool recheck)
        {
            ...
            Task<string> httpTasknew = httpClient.GetStringAsync("https://stand.sh/versions.txt");
            //Task<string> httpTaskOld = httpClient.GetStringAsync("https://stand.gg/versions.txt");
            DirectoryInfo bin_di = new DirectoryInfo(stand_dir + "\\Bin\\");
            string versions_string = "";
            try
            {
                httpTasknew.Wait();
                versions_string = httpTasknew.Result;
            }
            catch (Exception)
            {
                if (!recheck)
        ...
        private void Restart_click(object sender, EventArgs e)
        {
            ...
            if (gtaProcess != null)
            {
                try
                {
                    if (!gtaProcess.HasExited)
                    {
                        gtaProcess.Kill();
                        gtaProcess.WaitForExit(5000);
                    }
                }
                catch (Exception ex)
                {
                    Trace.WriteLine($"Erreur lors de la fermeture de GTA5: {ex}");
                }
            }
            ...
            // Réinitialiser les variables liées au processus GTA
            gta_pid = 0;
            game_was_open = false;
            ...
            // Réinjecter si le jeu a démarré
            if (gta_pid != 0)
            {
                inject();
                // Basculer les boutons
                toggleButtonsForRestart(true);

            }
            else
            {
                InfoText.Text = "Le jeu n'a pas démarré correctement. Veuillez réessayer.";
            }
        }
        private void LaunchGame()
        {
            switch (((DropDownEntry)LauncherType.SelectedItem).Id)
            {
                ...
                case (int)LauncherId.RSG:
                    ...
                    {
                        Trace.WriteLine($"Erreur lors du lancement de GTA5: {ex}");
                    }
                    break;
            }
        }
        ...
        {
            int attempts = 0;
            while (gta_pid == 0 && attempts < 60) // Attendre jusqu'à 30 secondes
            {
                updateGtaPid();
                if (gta_pid == 0)
                {
        ...
        public string Get8CharacterRandomString()
        {
            ...
            return path.Substring(0, random.Next(5, 12));  // Return 8 character string
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            string logPath = Path.Combine(stand_dir, "Log.txt");

            if (File.Exists(logPath))
            {
                try
                {
                    // Récupérer la liste actuelle des fichiers dans le presse-papiers
                    StringCollection currentClipboard = Clipboard.GetFileDropList();

                    // Vérifier si Log.txt est déjà dans le presse-papiers
                    if (!currentClipboard.Cast<string>().Any(path => Path.GetFileName(path).Equals("Log.txt", StringComparison.OrdinalIgnoreCase)))
                    {
                        // Ajouter Log.txt à la collection existante
                        currentClipboard.Add(logPath);

                        // Mettre à jour le presse-papiers
                        Clipboard.SetFileDropList(currentClipboard);

                        MessageBox.Show("Le fichier Log.txt a été ajouté au presse-papiers.", "Succès", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                    else
                    {
                        MessageBox.Show("Le fichier Log.txt est déjà dans le presse-papiers.", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                }
                catch (Exception ex)
                {
                    MessageBox.Show($"Impossible de copier le fichier Log.txt. Erreur : {ex.Message}", "Erreur", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
            else
            {
                MessageBox.Show("Le fichier Log.txt n'existe pas dans le répertoire spécifié.", "Erreur", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            {
                if (process.ProcessName == Process.GetProcessesByName("GTA5").FirstOrDefault().ProcessName)
                {
                    process.Kill();
                    break;
                }
        ...
        private void toggleButtonsForRestart(bool isRestarting)
        {
            LaunchBtn.Visible = !isRestarting;
            LauncherType.Visible = !isRestarting;
            InjectBtn.Visible = isRestarting;
            // Assurez-vous d'avoir un bouton "Kill" dans votre interface
            button2.Visible = isRestarting;
        }
    }
}
